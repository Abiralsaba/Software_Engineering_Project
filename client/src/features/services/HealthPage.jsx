...
export default function HealthPage() {
  ...
  return (
    <CitizenShell ministry="health" sections={sections} activeSection={section} onSectionChange={setSection}>

      <nav className="react-service-tabs nx-section-tabs" aria-label="Health sections">{sections.map(value => <button type="button" className={section === value ? 'active' : ''} onClick={() => setSection(value)} key={value}>{value.replace('-', ' ')}</button>)}</nav>
