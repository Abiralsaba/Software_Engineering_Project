...
import { MinistryActions, NidCardPreview } from '../../layouts/MinistryPresentation.jsx';
...
function ProfileTextFields({ fields, seed }) {
  return fields.map(([name, label]) => <label key={name}>{label}<input name={name} defaultValue={seed[name] || ''} type={name === 'email' ? 'email' : 'text'} /></label>);
}
...
export default function NidPage() {
  ...
  return <CitizenShell ministry="nid" sections={sections} activeSection={section} onSectionChange={setSection}>

    <nav className="react-service-tabs nx-section-tabs" aria-label="NID sections">{sections.map(value => <button type="button" className={section === value ? 'active' : ''} onClick={() => setSection(value)} key={value}>{value.replace('-', ' ')}</button>)}</nav>
    ...
    {loading ? <p className="react-empty-state">Loading NID services…</p> : <>
      {section === 'overview' && <><NidCardPreview profile={data.dashboard.profile} nid={nid} /><div className="react-service-stats">{[['Corrections', data.dashboard.stats?.corrections], ['Reissues', data.dashboard.stats?.reissues], ['Smart cards', data.dashboard.stats?.smart_cards], ['Address changes', data.dashboard.stats?.address_changes], ['Verifications', data.dashboard.stats?.verifications]].map(([label, value]) => <article className="react-panel" key={label}><strong>{value || 0}</strong><span>{label}</span></article>)}</div><MinistryActions ministry="nid" onSectionChange={setSection} /><section className="react-panel"><h2>{data.dashboard.profile?.name_en || 'Citizen NID profile'}</h2><p>NID: {nid || 'Not linked'} · Status: <StatusBadge value={data.dashboard.profile?.profile_status} /></p><HistoryCards items={rows(data.dashboard.recentApplications)} empty="No recent NID applications." title={row => `${row.type}: ${row.ref_no}`} /></section></>}

      {section === 'profile' && <section className="react-panel"><h2>Create or update NID profile</h2><form className="react-form-stack" encType="multipart/form-data" onSubmit={event => submit(event, '/profile', form => new FormData(form))}><fieldset className="nx-form-section"><legend>Personal information</legend><div className="react-form-grid"><ProfileTextFields seed={seed} fields={profileTextFields.filter(([name]) => !name.startsWith('present_') && !name.startsWith('permanent_'))} /><label>Date of birth<input name="date_of_birth" type="date" defaultValue={seed.date_of_birth ? String(seed.date_of_birth).slice(0, 10) : ''} required /></label><label>Gender<select name="gender" defaultValue={seed.gender || 'Male'}><option>Male</option><option>Female</option><option>Other</option></select></label><label>Blood group<select name="blood_group" defaultValue={seed.blood_group || ''}><option value="">Select</option>{['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'].map(value => <option key={value}>{value}</option>)}</select></label><label>Religion<input name="religion" defaultValue={seed.religion || ''} /></label></div></fieldset><fieldset className="nx-form-section"><legend>Present address</legend><div className="react-form-grid"><ProfileTextFields seed={seed} fields={profileTextFields.filter(([name]) => name.startsWith('present_'))} /><LocationIdFields apiBase={API} divisions={data.divisions} names={{ division: 'present_division_id', district: 'present_district_id', upazila: 'present_upazila_id' }} required={false} /></div></fieldset><fieldset className="nx-form-section"><legend>Permanent address</legend><div className="react-form-grid"><ProfileTextFields seed={seed} fields={profileTextFields.filter(([name]) => name.startsWith('permanent_'))} /><LocationIdFields apiBase={API} divisions={data.divisions} names={{ division: 'permanent_division_id', district: 'permanent_district_id', upazila: 'permanent_upazila_id' }} required={false} /></div></fieldset><fieldset className="nx-form-section"><legend>Supporting uploads</legend><div className="react-form-grid"><label>Photo<input name="photo" type="file" accept=".jpg,.jpeg,.png" /></label><label>Signature<input name="signature" type="file" accept=".jpg,.jpeg,.png" /></label></div></fieldset><p className="react-notice warning">The current backend accepts these upload fields but does not return proof that profile photo/signature paths were stored.</p><button className="btn-primary" disabled={submitting}>Save profile</button></form></section>}
